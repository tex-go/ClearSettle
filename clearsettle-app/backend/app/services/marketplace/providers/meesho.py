"""Meesho Provider — Manual Report Upload. Future: Meesho Supplier API."""
...
class MeeshoProvider(AbstractMarketplaceProvider):
    @property
    ...
    async def validate_connection(self, connection, credentials) -> ValidationResult:
        return ValidationResult(
            is_valid=True,
            message="Meesho is connected via manual report upload.",
            checks=[{"name": "manual_upload", "passed": True,
                     "detail": "Upload settlement reports from Meesho Supplier Panel."}],
        )

    async def get_account_details(self, connection, credentials) -> AccountDetails:
        return AccountDetails(
            account_id=connection.seller_id, account_name=connection.seller_name or "Meesho Seller",
    ...
    async def disconnect(self, connection, credentials) -> None:
        logger.info("Meesho connection %s disconnected.", connection.id)

    async def start_sync(self, connection, credentials, sync_job) -> SyncResult:
        return SyncResult(success=True, message="Meesho uses manual upload.",
                          meta={"upload_required": True})

    async def get_authorization_url(self, state, redirect_uri, connection, **kw) -> str:
        raise NotImplementedError("Meesho currently uses manual upload.")
