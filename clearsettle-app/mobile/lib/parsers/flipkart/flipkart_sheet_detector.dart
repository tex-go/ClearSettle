...
abstract final class FlipkartSheetDetector {
  ...
  static bool _isIgnored(String name) {
    ...
    // Flipkart non-order adjustment sheets — checked BEFORE _isOrders because
    // names like "non_order_spf" contain the substring "order" and would
    // otherwise be misclassified as order-level sheets.
    if (const [
      'non_order_spf', 'non_order', 'non-order',
      'spf_non',
      'neft_summary', 'neft summary',
      'ads', 'advertisement', 'campaign',
      'tcs_recovery', 'tds_recovery',
  ...
  static bool _isOrders(String name, List<String> headers) {
    const exactOrders = [
      'order', 'orders', 'payment', 'ledger', 'transaction', 'transactions',
    ];
    const phraseOrders = [
      'order p&l', 'order details', 'orders p&l', 'order level', 'order_level',
    ];

    for (final kw in exactOrders) {
      if (name == kw ||
          name.startsWith('${kw}_') ||
          name.startsWith('$kw ') ||
          name.endsWith('_$kw') ||
          name.endsWith(' $kw')) {
        return true;
